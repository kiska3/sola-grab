...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "{{sola-grab}}" project in the Warrior interface.
...
### For Debian/Ubuntu:
    ...
    pip install --upgrade seesaw
    su -c "cd /home/archiveteam; git clone https://github.com/kiska3/sola-grab.git; cd sola-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/sola-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, sola-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/sola-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/kiska3/sola-grab
    cd sola-grab; ./get-wget-lua.sh
